...
from walle.submit import get_env_var

from fabric import task, Connection, ThreadingGroup

# fab submit "scripts/runner_forces.py --dataset /home/scratch3/yogkr/GNN_Trainning/work_function/intraj/MD.db"

from walle._settings import (
    USER,
    HOSTS,
    REPO_PATH,
    CONDA_ENV,
...
if get_env_var('server_cfg') == 'FAB':
    from cfg.cfg import c
    USER = c.user
    REPO_PATH = c.git_remote
    CONDA_ENV = c.env
    ...
    NIFLHEIM_SCRIPT_PREAMBLE = f"""#!/bin/bash -ex
        #SBATCH --mail-type={c.slurm.mail_type}
        #SBATCH --partition={c.slurm.partition}
        #SBATCH -N {c.slurm.nodes}      # Minimum of 1 node
        #SBATCH -n {c.slurm.ntasks}     # 10 MPI processes per node
        #SBATCH --cpus_per_task 1     
        #SBATCH --time={c.slurm.time}
        #SBATCH --gres={c.slurm.gres}
        #SBATCH --job_name={c.exp_name}
...
def resume(c, timestamp, dataset_to_scratch=None):
    ...
    common_submit(
        new_script, commit_time_id, new_timestamp, output_path, dataset_to_scratch
    )
...
def qstatf(c):
    ...
    remote = Connection(NIFLHEIM_LOGIN_HOST, user=USER)
    remote.run(f"qstat -f  | grep -B 12 -A 5 'euser = {USER}'")
