...
with wContext(protext=True) as sweep:
    ...
...
subclasses = {k:[v_ for v_ in v if not k in v] for k,v in Config.items() if isinstance(v, wContext)}

Config = {k:v for k,v in Config.items() if not '__' in k} | subclasses
